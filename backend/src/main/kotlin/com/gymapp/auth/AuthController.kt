...
class BearerTokenFilter(private val jwt: JwtService) : OncePerRequestFilter() {
    override fun shouldNotFilter(request: HttpServletRequest) = !request.requestURI.startsWith("/api/v1/me") && !request.requestURI.startsWith("/api/v1/workout-plans") && !request.requestURI.startsWith("/api/v1/workout-sessions") && !request.requestURI.startsWith("/api/v1/curated-plans") && !request.requestURI.startsWith("/api/v1/training-summary") && !request.requestURI.startsWith("/api/v1/training-progress") && !request.requestURI.startsWith("/api/v1/body-measurements")
